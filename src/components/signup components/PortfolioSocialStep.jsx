...
const PortfolioSocialStep = ({ formData, setFormData, onNext, onPrev }) => {
  ...
  const [localUrl, setLocalUrl] = useState(
    (formData.portfolio && formData.portfolio.url) || ""
  );
  ...
  useEffect(() => {
    ...
    if (formData.portfolio) {
      if (typeof formData.portfolio === "string") {
        ...
        setLocalFile(formData.portfolio);
      } else if (formData.portfolio instanceof File) {
        const objUrl = URL.createObjectURL(formData.portfolio);
  ...
  const isPreviewImage = (() => {
    if (localFile && localFile.type?.startsWith("image/")) return true;
    if (typeof preview === "string") {
      return !!preview.match(/\.(jpeg|jpg|gif|png|webp|bmp|svg)$/i);
    }
    return false;
  })();
  ...
  return (
    <div className="w-full max-w-3xl bg-white rounded-lg p-6 border">
      ...
      {/* URL Input */}
      <div className="mb-4">
        <label className="block text-xs font-semibold">{t("portfolio_social.url_label")}</label>
        <input
          ref={urlInputRef}
          type="url"
      ...
      {Object.keys(error).length > 0 && (
        <div className="mt-6 text-xs text-red-600 bg-red-50 border border-red-300 rounded-md p-3">
