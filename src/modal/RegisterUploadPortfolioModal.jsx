...
import { updatePortfolio } from "../Hooks/useAuth";

const RegisterUploadPortfolioModal = ({ isOpen, onClose, initialData = {}, onSaved }) => {
  ...
  useEffect(() => {
    ...
    setForm({
      title: initialData?.data?.title ?? initialData.title ?? "",
      description: initialData?.data?.description ?? initialData.description ?? "",
      role: initialData?.data?.role ?? initialData.role ?? "",
      technologies: initialData?.data?.technologies ?? initialData.technologies ?? "",
      project_url: initialData?.data?.project_url ?? initialData.project_url ?? "",
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.title && files.length === 0 && existingMedia.length === 0) {
      return setErrors("Nothing to save");
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl overflow-auto max-h-[90vh]">
        <div className="p-4 border-b flex justify-between items-center">
          ...
          <button type="button" onClick={onClose} className="text-gray-500"><FaTimes /></button>
        </div>
...
export default RegisterUploadPortfolioModal;
