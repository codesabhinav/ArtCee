...
const BusinessDirectory = () => {
  // only need translation function `t` here (language switching UI removed)
  ...
  // default sort labels use translation strings
  ...
  const [sort, setSort] = useState(defaultSortOptions[0]);
  ...
  // load filters (re-run when translations update)
  useEffect(() => {
    (async () => {
      try {
        const meta = await getBusinessFilters();
        ...
        const humanLabel = (key) => {
          if (key === "type") return t("filters.type_label");
          if (key === "locations") return t("filters.location_label");
          return key;
        };
        ...
        if (meta?.order_by_rate) {
          ...
          labelToKey["__order_by_map__"] = map;
          // prefer API-provided labels if present, otherwise keep translated defaults
          if (sortLabels.length > 0) {
            setSortOptions(sortLabels);
            setSort(sortLabels[0]);
          } else {
            setSortOptions(defaultSortOptions);
            setSort(defaultSortOptions[0]);
          }
        } else {
          setSortOptions(defaultSortOptions);
          setSort(defaultSortOptions[0]);
    ...
    // include `t` so filters re-evaluate when translations change
  }, [t]);
  ...
  const buildApiParams = () => {
    ...
    Object.entries(selectedFilters).forEach(([label, chosenLabel]) => {
      ...
      // simpler, safer check: if option text begins with the English "All" or localized "All"
      const allCandidates = [
        "All",
        t("filters.all_types"),
        t("filters.all_locations"),
        t("filters.all"), // generic if present
      ]
        .filter(Boolean)
        .map((s) => s.toString().toLowerCase());

      const chosenLower = chosenLabel.toString().toLowerCase();
      if (allCandidates.some((p) => chosenLower.startsWith(p))) return;

      const filterObj = filtersConfigDynamic.find((f) => f.label === label);
      if (!filterObj) return;

      const apiKeyFromLabel = filterLabelToKeyMap[chosenLabel];
      if (!apiKeyFromLabel) return; // can't map -> skip

      params[filterObj.key] = apiKeyFromLabel;
    });

    // handle sort mapping using special order_by_map__
    if (sort && filterLabelToKeyMap["__order_by_map__"]) {
      const orderMap = filterLabelToKeyMap["__order_by_map__"];
      const mapped = orderMap[sort];
      if (mapped) params.order_by = mapped;
  ...
  // fetch businesses when deps change
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
    ...
    // if we have filter config, use buildApiParams; otherwise fetch by search only
    if (filtersConfigDynamic.length > 0) {
      fetchData();
    } else {
      (async () => {
  ...
  }, [debouncedSearch, sort, selectedFilters, filtersConfigDynamic, t]);

  const clearFilter = (label) => {
    setSelectedFilters((prev) => ({
      ...prev,
      [label]:
        defaultSelectedFilters[label] ||
        (prev[label] && prev[label].toString().startsWith("All") ? prev[label] : ""),
    }));
  };
    ...
    <div className="bg-white min-h-screen w-full">
      <div className="md:max-w-[80%] mx-auto">
        ...
        <div className="flex md:flex-row md:items-center md:justify-between md:px-0 px-4 py-4 gap-3 md:gap-0">
          ...
          <h1 className="text-center text-lg md:text-xl font-bold">
            {t("ui.business_directory")}
          </h1>

          <button className="px-4 md:px-6 py-2 text-xs bg-teal-500 text-white rounded-md mx-auto md:mx-0">
            {t("ui.industry_partners")}
          </button>
        </div>
        ...
        <div className="md:px-0 px-4 py-4 space-y-4 border-b">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:flex md:space-x-3 gap-3">
            {/* NOTE: Language dropdown removed */}

            {/* Type dropdown */}
            <CustomDropdown
              options={
                filtersConfigDynamic.find((f) => f.key === "type")?.options ||
                [t("filters.all_types"), "CPA", "Law Firm", "Consulting"]
              }
              value={
                selectedFilters[t("filters.type_label")] ||
                (filtersConfigDynamic.find((f) => f.key === "type")?.options?.[0] ??
                  t("filters.all_types"))
              }
              setValue={(val) =>
                setSelectedFilters((prev) => ({ ...prev, [t("filters.type_label")]: val }))
              }
            />
            ...
            <CustomDropdown
              options={
                filtersConfigDynamic.find((f) => f.key === "locations")?.options ||
                [t("filters.all_locations"), "New York, NY", "Los Angeles, CA", "Miami, FL"]
              }
              value={
                selectedFilters[t("filters.location_label")] ||
                (filtersConfigDynamic.find((f) => f.key === "locations")?.options?.[0] ??
                  t("filters.all_locations"))
              }
              setValue={(val) =>
                setSelectedFilters((prev) => ({ ...prev, [t("filters.location_label")]: val }))
              }
            />
        ...
        <div className="px-4 md:px-0 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
            <p className="text-sm text-gray-500">
              {t("ui.showing_businesses", { count: showingCount })}
            </p>
            <span className="bg-orange-400 font-bold text-white text-xs px-3 py-1 rounded-md text-center sm:text-left">
              {t("ui.verified_partners")}
            </span>
          </div>

          {loading && (
            <p className="text-gray-500">
              <SpinnerProvider /> {t("ui.loading")}
            </p>
          )}
          {error && <p className="text-red-500">{error}</p>}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {/* Price */}
                    <p className="mt-3 text-gray-800 font-medium text-sm">
                      💲 {t("pricing.hourly")}: ${biz.hourly_rate} | {t("pricing.daily")}: $
                      {biz.daily_rate} | {t("pricing.project")}: ${biz.project_rate}
                    </p>
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:space-x-3 justify-between">
                      <a
                        ...
                        className="px-3 py-2 border rounded-md text-xs flex items-center justify-center gap-2 hover:bg-gray-100"
                      >
                        <FaGlobe /> <span>{t("ui.website")}</span>
                      </a>
                      <button className="sm:ml-auto px-3 py-2 bg-teal-500 text-white font-semibold text-xs rounded-md hover:bg-teal-600">
                        {t("ui.view_details")}
                      </button>
