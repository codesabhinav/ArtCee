...
const pushFilter = (keyName, apiObj, allLabel) => {
  ...
  // Map using the KEYS, not values
  const options = Object.keys(apiObj).map((k) =>
    t(`creative.filter_options.${keyName}.${k}`, { defaultValue: apiObj[k] })
  );

  cfg.push({ label: humanLabel(keyName), options: [allLabel, ...options], key: keyName });

  // Map back translated label → API key
  Object.keys(apiObj).forEach((k) => {
    const translated = t(`creative.filter_options.${keyName}.${k}`, { defaultValue: apiObj[k] });
    labelToKey[translated] = k;
  });
};
    ...
    // depend on t so labels update when language changes
  }, [t]);
  ...
  const getAvailability = (creative) => {
    const status = (creative?.status || "").toLowerCase();
    if (status === "online" || status === "available") return { label: t("creative.available"), badge: "bg-green-100 text-green-700" };
    if (status === "busy") return { label: t("creative.busy"), badge: "bg-yellow-100 text-yellow-700" };
    if (status === "booked") return { label: t("creative.booked"), badge: "bg-orange-100 text-orange-700" };
    return { label: t("creative.offline"), badge: "bg-gray-100 text-gray-700" };
  };
  ...
  const SidebarFilters = () => {
    // Try to read a default industries array from translations (supports returnObjects)
    const defaultIndustriesRaw = t("creative.default_industries", { returnObjects: true });
    const defaultIndustries = Array.isArray(defaultIndustriesRaw) ? defaultIndustriesRaw : ["Industries", "Services", "Skills"];
          ...
          <div className="mt-2 space-y-2 text-xs">
            {filtersMeta?.industries
              ? Object.values(filtersMeta.industries).map((ind, i) => {
                  // try to localize API-provided label using a translation map creative.industry_map.<apiLabel>
    ...
    <div className="bg-white min-h-screen w-full">
      <div className="md:max-w-[80%] justify-center mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between px-2 md:px-0 py-4 gap-3">
          ...
          <h1 className="text-center text-lg md:text-xl font-bold flex-1">{t("creative.title")}</h1>
          <button className="px-4 py-2 text-xs bg-teal-500 text-white rounded-md  hidden lg:block">
            {t("creative.count_creatives", { count: creatives.length })}
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 w-full">
            {loading ? (
              ...
              creatives.map((creative) => {
                ...
                const years = creative?.experience_in_year ?? "0";
                const level = creative?.experience_in_level
                  ? t(`creative.levels.${creative.experience_in_level}`, { defaultValue: creative.experience_in_level })
                  : t("creative.levels.unknown");
                const workStyle =
                  creative?.on_site_active === "1" && creative?.is_remote_active === "0" 
                    ? t("creative.on_site")
                    : creative?.is_remote_active === "1" && creative?.on_site_active === "0"
